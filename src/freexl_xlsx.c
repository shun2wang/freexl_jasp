...
/ 
/ Portions created by the Initial Developer are Copyright (C) 2021-23
...
static void
destroy_workbook_rel (xlsx_workbook_rel * wr)
{
/* memory cleanup - destroying a WorkbookRel object */
    if (wr->id != NULL)
	free (wr->id);
    if (wr->target != NULL)
	free (wr->target);
...
/* memory cleanup - destroying a WorkBook object */
    xlsx_workbook_rel *wr;
    xlsx_workbook_rel *wr_n;
    xlsx_worksheet *ws;
    xlsx_worksheet *ws_n;
    xml_datetime *date;
    xml_datetime *date_n;
    ...
    if (wb->styles != NULL)
	free (wb->styles);
    if (wb->WorkbookRelsZipEntry != NULL)
	free (wb->WorkbookRelsZipEntry);
...
/* parsing a Zipfile directory */
    ...
    for (i = 0; i < gi.number_entry; i++)
	  ...
	  if (strcasecmp (filename, "xl/_rels/workbook.xml.rels") == 0)
	    {
		/* found WorkbookRels */
		workbook->WorkbookRelsZipEntry = setString (filename);
	    }
		...
		if (col_no >= 0)
		  {
		      ...
		      if (t != NULL)
			{
				 ...
				 * sandro 2023-09-15
				 * 
				 * sometimes text strings are declared inlined
				 * instead of using the SharedStrings index
				 *
				 */
			    if (strcmp (t, "str") == 0)
				type = XLSX_STRING;
...
/* uncompressing SheetN.xml */
    ...
    if (worksheet->target != NULL)
	  ...
	  /* 
	   * 2023-08-14 sandro
	   * completing the patch suggested by Mark Finnis
	   * 
      ...
      {
	  zip_entry = malloc (strlen ("xl/worksheets/sheet123456789.xml") + 1);
...
static void
do_add_worksheet (xlsx_workbook * workbook, int id, char *name, char *target)
{
...
static char *
resolve_rel (xlsx_workbook * workbook, const char *id)
{
/* attempting to resolve a WorkbookRel */
    xlsx_workbook_rel *rel = workbook->first_rel;
    while (rel != NULL)
      {
	  if (strcmp (rel->id, id) == 0)
	    {
		char *name = NULL;
		if (rel->target == NULL)
		    return NULL;
		name = malloc (strlen (rel->target) + 1);
		strcpy (name, rel->target);
		return name;
	    }
	  rel = rel->next;
      }
...
/* 
 * 2023-08-14 
 * patch suggested by Mark Finnis and implemented by Sandro 
*/
		      if (strcmp (k, "r:id") == 0)
			{
			    /* Needs to be resolved through _rels/workbook.xml.rels */
			    id = atoi (v + 3);
			    target = resolve_rel (workbook, v);
			}
/* end Mark Finnis and Sandro */
	  ...
	  if (id > 0 && name != NULL)
	      do_add_worksheet (workbook, id, name, target);
	  else
	    {
		if (name != NULL)
		    free (name);
...
static void
add_workbook_rel (xlsx_workbook * workbook, const char *id, const char *target)
{
/* adding a WorkbookRel to the list */
    int len;
    xlsx_workbook_rel *rel = malloc (sizeof (xlsx_workbook_rel));
    len = strlen (id);
    rel->id = malloc (len + 1);
    strcpy (rel->id, id);
    len = strlen (target);
    rel->target = malloc (len + 1);
    strcpy (rel->target, target);
    rel->next = NULL;

    if (workbook->first_rel == NULL)
	workbook->first_rel = rel;
    if (workbook->last_rel != NULL)
	workbook->last_rel->next = rel;
    workbook->last_rel = rel;
}

static void
rels_start_tag (void *data, const char *el, const char **attr)
{
/* some generic XML tag starts here */
    const char **attrib = attr;
    int count = 0;
    ...
    const char *v;
    xlsx_workbook *workbook = (xlsx_workbook *) data;
    if (strcmp (el, "Relationships") == 0)
	workbook->RelsOk = 1;
    if (strcmp (el, "Relationship") == 0)
      {
	  if (workbook->RelsOk == 1)
	    {
		const char *id = NULL;
		...
		if (id != NULL && target != NULL)
		    add_workbook_rel (workbook, id, target);
	    }
	  else
	      workbook->error = 1;
      }
...
static void
rels_end_tag (void *data, const char *el)
{
/* some generic XML tag ends here */
    xlsx_workbook *workbook = (xlsx_workbook *) data;
    if (strcmp (el, "Relationships") == 0)
...
static void
do_parse_xlsx_workbook_rels (xlsx_workbook * workbook, unsigned char *buf,
			     uint64_t size_buf)
{
/* parsing WorkbookRels */
    XML_Parser parser;
    int done = 0;
    ...
    XML_SetUserData (parser, workbook);
    XML_SetElementHandler (parser, rels_start_tag, rels_end_tag);
    XML_SetCharacterDataHandler (parser, xmlCharData);
    if (!XML_Parse (parser, (char *) buf, size_buf, done))
	workbook->error = 1;
    XML_ParserFree (parser);
...
static void
do_fetch_xlsx_workbook_rels (unzFile uf, xlsx_workbook * workbook)
{
/* uncompressing WorkbookRels */
    int err;
    char filename[256];
    unz_file_info64 file_info;
    uint64_t size_buf;
    ...
    err = unzLocateFile (uf, workbook->WorkbookRelsZipEntry, 0);
    if (err != UNZ_OK)
      {
	  workbook->error = 1;
	  goto skip;
...
/* parsing WorkbookRels */
    do_parse_xlsx_workbook_rels (workbook, buf, size_buf);
...
/* parsing the Zipfile directory */
    ...
    if (workbook->WorkbookRelsZipEntry != NULL)
      {
	  /* parsing workbook.xml.rels */
	  do_fetch_xlsx_workbook_rels (uf, workbook);
	  if (workbook->error)
	    {
		destroy_workbook (workbook);
		retval = FREEXL_INVALID_XLSX;
